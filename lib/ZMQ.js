...
class ZmqExporter extends Exporter {
	...
	init(options) {
		...
		return new Promose((resolve, reject) => {
			if (!options.zmqPort || typeof options.zmqPort !== 'number') {
				return reject(new Error('ZMQ port not provided'));
			}
			
			this._sock.bind('tcp://127.0.0.1:' + options.zmqPort, err => {
	...
	export(tx, trytes, index, isIncluded) {
		...
		if (typeof index === 'boolean' && index) || isIncluded) {
