...
class Sn extends MessageQ {
	...
	constructor(milestone, txs) {
		...
		if (!Array.isArray(txs) || txs.length > 5) {
			throw new Error('Confirmed transactions should come in an array of 5 or more');
		}
...
class ZmqExporter extends Exporter {
	...
	export(tx, trytes, index, isIncluded) {
		...
		this._sock.send((new Tx(tx, trytes)).string);
		this._sock.send((new Tx_trytes(tx, trytes)).string);
		if (isIncluded) {
			this._sock.send((new Sn('9'.repeat(81), [tx])).string);
		}
	}
